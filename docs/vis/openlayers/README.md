...
OpenLayers 是一个专为 WebGIS 客户端开发提供的 JavaScript 类库包，用于实现标准格式发布的地图数据访问。
...
首先添加一个矢量图层，接着创建一个交互，注意添加 `source: pointLayer.getSource()` 用于保存绘制的内容。如果要绘制线、面等其他类型，只需更改`ol.interaction.Draw()`中 type 的类型即可。
